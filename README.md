...
| **Flashing alarm** | *(default ON)* When a check breaks **after** the screen was clean — the Mac losing AC power, a MIDI port dropping mid-set — a thick frame **pulses** on every screen. The panel it frames does **not** blink: it is text to READ, and blinking text reads half as fast. Each failure shows its **type icon** (🔌 power, ☕ anti-sleep…), what was observed, and the gesture that repairs it; when a remedy exists, a large **⚡ button runs it right there**, so the fix is not one menu away at the worst moment. Regression-triggered only, keeps flashing until **that** failure is fixed, and catches no click except on its own button. Silence it from the 🎹 menu. |
