...
struct AlarmState {
    ...
    mutating func update(status: RigStatus, problems: [Problem]) {
        ...
        // Un « tais-toi » ne vaut que pour les pannes qu'on connaissait en le prononçant.
        // Ce qui casse ENSUITE a droit à son clignotement — y compris pendant qu'une autre
        // panne est reportée. La version d'avant exigeait en plus que l'ardoise soit VIDE
        // (`keys.isEmpty`), ce qui rendait la promesse fausse dans le seul cas où elle
        // compte : on tait le Stream Deck pour finir le morceau, l'alimentation lâche
        // pendant ce temps, et l'écran reste muet. Constaté sur banc d'essai le 29/08.
        if !fresh.isEmpty { silenced = false; snoozedUntil = nil }
        keys.formUnion(fresh)
        keys.formIntersection(now)          // réparé → sort de l'alarme
        // Plus rien de cassé : ni silence ni report n'ont d'objet, et les garder ferait
        // taire la prochaine panne au motif d'une décision prise sur une autre.
...
final class BigButton: NSButton {
    ...
    func set(title: String, subtitle: String) {
        ...
        // Le contraste d'un bouton éteint se règle vers le HAUT, pas vers le bas. « Pas de
        // correctif auto » est une réponse, pas une absence : c'est même la ligne la plus
        // utile du panneau quand elle s'affiche, puisqu'elle dit qu'il faut se lever.
